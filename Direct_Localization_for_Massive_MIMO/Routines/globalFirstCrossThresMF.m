function [ TOAs, snapshot ] = globalFirstCrossThresMF(receivedSignals,bandwidth,oversamplingFactor,startIndex,noisePower)
...
% -----------------------------------------------------------
    ...
    for l=1:numBS
        energyDetect = sum(abs(receivedSignals{l}).^2,1) >= thres(l);
%         % TEST
%         plot(sum(abs(receivedSignals{l}).^2,1))
%         thres(l)
%         % TEST
        if sum(energyDetect)>0
            ind = find(energyDetect,1,'first');
            TOAs(l) = (ind-startIndex)/oversamplingFactor/bandwidth;
            snapshot{l} = receivedSignals{l}(:,ind);
        end
    end
