...
function K_Gas = Gas_Abs_Coef(p, data)
% =========================================================================
...
f = repmat(reshape(p.fk,[],1),1,length(data.fc0));   % Frequency
...
fc = repmat((data.fc0+data.delta*(p.p/p.p0))',p.K,1);
...
alpha = repmat((((1-data.q).*(data.alphaAir)+data.q.*(data.alphaGas)).*((p.p/p.p0)*((p.T0/p.T).^data.gamma)))',p.K,1);
